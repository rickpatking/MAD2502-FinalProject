...
def build_weighted_graph(vor: Voronoi, sites: np.ndarray) -> dict:
    ...
    The penalty is inversely proportional to the clearance from the sites—edges whose midpoints are close
...
def dijkstra(start_vertex: int, stop_vertex: int, vor: Voronoi, sites: np.ndarray) -> list:
    """
    ...
    connected_vertices = build_weighted_graph(vor, sites)
...
def path_finder(start: np.ndarray, stop: np.ndarray, vor: Voronoi, sites: np.ndarray):
    """
    ...
    vertex_path = dijkstra(starting_vertex[1], stopping_vertex[1], vor, sites)
    full_path = []
